...
Welcome! With this project, I hope to provide a clear demonstration of my capabilities. While I don't claim to be an expert in linear programming nor a full-blown developer, I do posess the curiosity and intelligence to deliver decision-useful projects and analysis.
...
# NEED SOME EXPLANATION OF THE BL MODEL HERE
...
for(i in 1:90){
  for(k in 1:ncol(R)){
    ...
    R[i,k] <- R[i,k] * (1 + rnorm(1,drift,1)/100)
      ...
      R[i,k] <- R[i-1,k] * (1 + rnorm(1,drift,1)/100)
    }
    }
}
...
# We won't pass Sigma or Omega to the optimization, allowing the function to use defaults
Omega <- matrix(0, nrow = length(Views), ncol = length(Views))
diag(Omega) <- runif(length(Views),0,.1) #1
```

We now utilize R's PortfolioAnalytics package to optimize via the Black-Litterman model. We first setup a base portfolio with the only constraints being that it is Long-only and that the portfolio is fully invested. Note that the leverage constraint allows for some leeway when utilizing other optimizers, such as Differential Evolution. I don't use that in this demonstration but may have to do so if I add a Duration objective. Stay tuned for that!
...
Next, I intend to add a duration target to stay within a reasonable range from the index's initial value. This should offer an even more practical solution. However, this involves the use of a differ solver that has been fighting back...to be continued.
