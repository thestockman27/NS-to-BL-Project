...
At this time, I am reviewing the output of the optimizations and have noticed some inconsistencies regarding the returns calculated by the Black-Litterman model. The current obstacle is in familiarizing myself with some augmentation to the matrix algebra that the authors of the PortfolioAnalytics package made to the original Black-Litterman formula. When spot-checking the output by hand with the original formula, I arrive at slightly different results, so this issue has slowed progress. 

Further, there are some computing limitations. I am running this on my laptop and simply don't have the processing power to perform some of the larger optimizations later on in the project. There also seems to be an issue with my system assuming that large matrices are invertible due to the determinant being near zero but not actually zero. **These constraints have forced me to shrink the number of assets to the first 60 in the index.**

With all that said, let's continue.

#### Outline 
  The main idea of this project is to provide a methodology for an active manager to identify relative value opportunities along the curve and then construct an optimal portfolio based on those opportunites. It was was inspired by my work with factor modeling in corporate bond markets. My research uncovered the persistence of a Value factor in determining excess returns, particularly within the Investment Grade universe. Measurement of Value must account for the term structure of interest rates and spread. It is preferable to perform this analysis upon the Option Adjusted Spread (OAS) of individual bonds, unfortunately I was unable to locate publicly available OAS data, so we will settle for using the Yield-to-Worst (YTW) statistic.
...
black.litterman.corrected <- function (R, P, Mu = NULL, Sigma = NULL, Views = NULL, Omega = NULL) 
{
  ...
  B = BlackLittermanFormula(Mu, Sigma, P, v = Views, Omega)
  return(B)
}
...
Further, the confidence level of each view is the most difficult parameter to estimate.We will operate under the very optimistic assumption that we have 100% confidence in our views, or rather, there is no variance in our confidence. This is represented by setting the diagonal elements of the Omega matrix equal to Zero. Essentially, the higher the variance in each asset, the less confident we are in the view provided by the Nelson-Siegel model.
...
#NS_output <- subset(NS_output, Sector == 'Basic Industry')
NS_output <-NS_output[1:60,]
...
```
```{r,echo=FALSE}
plot.xts(R[,c(1:3,12,14:19)], main = "Random Walk w/ Drift in Cons. Non-Cyc.", xlab = "Date", ylab = "Yield-to-Worst (%)")
```
...
diag(Omega) <- 0 #runif(length(Views),0,.3) #1
```

We now utilize R's PortfolioAnalytics package to optimize via the Black-Litterman model. We first setup a base portfolio with the following constraints: 
- it is Long-only
...
bl1.portf <- add.constraint(portfolio = bl1.portf, type = "box",
                            min= min(NS_output$`Weight (%)`)/100,max = .1,min_mult =NULL, max_mult=NULL )
...
Let's explore our results! It appears that there is potential value within the Communications Sector. While this quantitative approach is not all-inclusive, the identification of opportunities within these sectors allows credit analysts to better prioritize their research efforts. Potentially allowing the team to capture even more alpha.
```{r,echo=FALSE}
...
init_sector_wts <- foreach(b = unique(NS_output$Sector)) %do%{
  ...
  sum(s$`Weight (%)`) / sum(NS_output$`Weight (%)`)
}
...
weights <- data.frame(
  ...
  Initial_Weight = unlist(init_sector_wts),
...
#### Third Portfolio ####
# Add duration target
# NS_output$New_Weight <- NS_output$`Weight (%)` / sum(NS_output$`Weight (%)`)
# 
# dur_target <- round(sum(NS_output$Duration * NS_output$New_Weight),2)
# 
# absolute_duration_deviation <- function(R, weights,holdings,init_port_dur){
#           
#           durations <- holdings$Duration[1:ncol(R)]
#           init_weights <- holdings$New_Weight[1:ncol(R)]
...
#                            arguments=list(holdings = NS_output, init_port_dur = dur_target))
# library(DEoptim)
# library(foreach)
